...
- 解锁审批：针对连续输错密码导致的锁定，支持带原因、有效期和会话范围的解锁申请；由不同于申请人的安全管理员审批，高权限账号需第二人确认；批准后仅清除本次锁定并按范围失效旧会话，不改动密码与长期账号状态；审批人权限在完成前失效会终止申请。
...
测试覆盖身份初始化、登录、用户与角色维护、权限计算、账号停用后的会话撤销、审计脱敏、解锁申请审批（含双人确认、权限失效终止、唯一终态与首次登录对照）、居民事务、信访状态流转、公告排序、后台任务去重与领取，以及数据库时间格式。
